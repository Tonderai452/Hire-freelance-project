...
class WC_Gateway_SnapScan extends WC_Payment_Gateway {
	...
	public function admin_options() {
		...
		print '<br>';
		print __( 'To get started you will need an API Key, Token and a SnapCode, you can email <a href="mailto:woocommerce@snapscan.co.za">woocommerce@snapscan.co.za</a> for your free key.', 'woocommerce-gateway-snapscan' );
		print '<br>';
		print  __( 'Be sure to mention you use the <b>SnapScan WooCommerce plugin</b> and your url is <b>' . home_url( '/' ) . '</b>.', 'woocommerce-gateway-snapscan' );
		print "</p>";
		...
		if ( 'ZAR' == get_option( 'woocommerce_currency' ) ) {
			...
			print sprintf( __( 'Choose South African Rands as your store currency in %1$sPricing Options%2$s to enable the SnapScan Gateway.', 'woocommerce-gateway-snapscan' ), '<a href="' . esc_url( $url ) . '">', '</a>' );
			print '</p></div>';
		}
	}
	...
	function receipt_page( $order_id ) {
		...
		$id             = $order->id;
		$qr_url         = 'https://pos.snapscan.io/qr/' . $snapcode . '?id=' . $id . '&strict=true&amount=' . $total_in_cents;
		$qr_image_url   = 'https://pos.snapscan.io/qr/' . $snapcode . '.png?id=' . $id . '&strict=true&amount=' . $total_in_cents;
	...
	function api_callback() {
		...
		if ( isset( $_GET['check_status'] ) && $_GET['check_status'] ) {
			...
			if ( ! in_array( $order->get_status(), array( 'pending', 'failed' ) ) ) {
				...
				$last_poll_time = get_post_meta( $order->id, 'snapscan_poll_request', true );
				if ( $last_poll_time === '' || ( time() - $last_poll_time > 5 ) ) {
					update_post_meta( $order->id, 'snapscan_poll_request', time() );
	...
	function successful_request( $payload ) {
		...
		if ( ( 'completed' === $payload['status'] ) && ( ! in_array( $order->get_status(), array(
			...
			delete_post_meta( $order->id, 'snapscan_poll_request' );
		}
	}
